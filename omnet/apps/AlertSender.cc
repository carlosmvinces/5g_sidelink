...
void AlertSender::sendAlertPacket()
{
    ...
    if (nextSno_ == 0 && !hasCrashed_) {
        veins::VeinsInetMobility* traciMob = dynamic_cast<veins::VeinsInetMobility*>(mobility);

        if (traciMob != nullptr) {
            EV << "SAPA: car[0] sufriendo percance. Deteniendo vehiculo y enviando alerta." << endl;
            traciMob->getVehicleCommandInterface()->setSpeed(0);
            traciMob->getVehicleCommandInterface()->setColor(veins::TraCIColor(255, 165, 0, 255)); // Naranja
        }

        inet::Coord currentPos = mobility->getCurrentPosition();

        crashX_ = currentPos.x;
        crashY_ = currentPos.y;
        hasCrashed_ = true;
    }

    if (hasCrashed_) {
            alert->setSenderPosX(crashX_);
            alert->setSenderPosY(crashY_);
        }
    else {
            alert->setSenderPosX(currentPos.x);
            alert->setSenderPosY(currentPos.y);
        }
